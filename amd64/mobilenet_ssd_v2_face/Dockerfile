...
# 1) build: docker build -t "neuralet/amd64:mobilenet_ssd_v2_face" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/amd64:mobilenet_ssd_v2_face
...
WORKDIR /repo/amd64/mobilenet_ssd_v2_face
