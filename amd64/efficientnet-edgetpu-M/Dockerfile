...
# 1) build: docker build -t "neuralet/amd64:efficientnet-edgetpu-M" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/amd64:efficientnet-edgetpu-M
...
WORKDIR /repo/amd64/efficientnet-edgetpu-M
