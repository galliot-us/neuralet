...
# 1) build: docker build -t "neuralet/amd64:inception_v3_quant" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/amd64:inception_v3_quant
...
WORKDIR /repo/amd64/inception_v3_quant
