...
# 1) build: docker build -t "neuralet/amd64:mobilenet_ssd_v1" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/amd64:mobilenet_ssd_v1
...
WORKDIR /repo/amd64/mobilenet_ssd_v1
