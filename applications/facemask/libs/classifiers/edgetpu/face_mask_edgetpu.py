...
class Classifier:
    ...
    def __init__(self, config):
        ...
        self.model_path = 'libs/classifiers/edgetpu/' + self.model_file
        # Frames Per Second
        self.fps = None
        if not os.path.isfile(self.model_path):
            url= "https://media.githubusercontent.com/media/neuralet/neuralet-models/master/edge-tpu/OFMClassifier/OFMClassifier_edgetpu.tflite"
...
def inference(self, resized_rgb_image) -> list:
        ...
        result = list(np.argmax(np.array(net_output), axis=1))  # returns class id

        # TODO: optimized without for
        scores = []
        for i, itm in enumerate(net_output):
