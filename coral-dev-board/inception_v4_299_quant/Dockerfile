...
# 1) build: docker build -t "neuralet/coral-dev-board:inception_v4_299_quant" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/coral-dev-board:inception_v4_299_quant
...
WORKDIR /repo/coral-dev-board/inception_v4_299_quant
