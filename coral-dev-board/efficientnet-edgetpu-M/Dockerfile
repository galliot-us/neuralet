...
# 1) build: docker build -t "neuralet/coral-dev-board:efficientnet-edgetpu-M" .
# 2) run: docker run -it --privileged --net=host -v /PATH_TO_CLONED_REPO_ROOT/:/repo neuralet/coral-dev-board:efficientnet-edgetpu-M
...
WORKDIR /repo/coral-dev-board/efficientnet-edgetpu-M
